...
describe 'Installing ServiceControl and configuring Error, Audit, and Monitoring Instance' do
  ...
  before(:all) do
    @older_package_version = run_shell(interpolate_powershell(get_version_before_latest_of_choco_package('servicecontrol'))
  end
  ...
  context 'with an updated version of the package' do
    before(:all) do
      @latest_package_version = run_shell(interpolate_powershell(get_latest_version_of_choco_package('servicecontrol'))
    end
